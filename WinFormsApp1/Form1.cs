...
namespace WinFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        private Cancha? ObtenerCanchaPorTipoSeleccionado()
        {
            ...
            var cancha = db.Canchas.Where(c => c.Activa && c.Tipo == tipo).OrderBy(c => c.Nombre).FirstOrDefault();
            return cancha;
        }

        private void cbCancha_SelectedIndexChanged(object sender, EventArgs e)
        {
            // No-op: single combo used; selection evaluated on reservar
        }
        ...
        private void btnRegistrar_Click(object sender, EventArgs e)
        {
            ...
            if (cancha is null)
            {
                MessageBox.Show("No hay canchas activas para el tipo seleccionado.");
                return;
            }
            if (socio is null)
            {
